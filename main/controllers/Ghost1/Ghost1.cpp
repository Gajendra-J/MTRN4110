...
private:
  ...
  DistanceSensor *_ps7;
  Field *_translation;
  Field *_targetTranslation;
  Field *_ghost2Translation;
    ...
    : _mapFileName(mapFileName), _wheelRadius(wheelRadius), _axleLength(axleLength)
{
  ...
  _startHeading = 'N';
  _translation = _robot->getFromDef("Ghost_1")->getField("translation");
  _targetTranslation = _robot->getFromDef("Player")->getField("translation");
  _ghost2Translation = _robot->getFromDef("Ghost_2")->getField("translation");
...
void Epuck::readMap() {
  ...
  // Hardcode teleport zones
  _adjacencyMatrix.at(171).at(3) = 1;
  _adjacencyMatrix.at(189).at(1) = 1;
}

void Epuck::getPosition() {
  _start = calculateCell(_translation->getSFVec3f());
  _end = calculateCell(_targetTranslation->getSFVec3f());
  _ghost2 = calculateCell(_ghost2Translation->getSFVec3f());
...
void Epuck::move1Step()
{
  ...
  do
  {
    ...
    if (checkCollision(leftEffective - currLeftPos, rightEffective - currRightPos)) break;
  } while (fabs(newLeftPos - leftEffective) > STEPDELTA || fabs(newRightPos - rightEffective) > STEPDELTA);
}

bool Epuck::checkCollision(double left, double right) {
  const auto ps0 = _ps0->getValue();
  const auto ps1 = _ps1->getValue();
  const auto ps6 = _ps6->getValue();
  const auto ps7 = _ps7->getValue();
  //std::cout << "Distance Sensors: " << std::to_string(ps0) << ", " << std::to_string(ps1) << ", " << std::to_string(ps6) << ", " << std::to_string(ps7) << std::endl;
  if (ps0 > 90 || ps1 > 90 || ps6 > 90 || ps7 > 90) {
    const auto values = _ghost2Translation->getSFVec3f();
    const auto curr = _translation->getSFVec3f();
    const auto distance = std::sqrt(std::pow(values[0] - curr[0], 2) + std::pow(values[2] - curr[2], 2));
    if (distance <= 0.1) {
      reverse(left, right);
      return true;
    }
...
void Epuck::reverse(double leftPos, double rightPos) {
  auto currLeftPos = _leftPosSensor->getValue();
  auto currRightPos = _rightPosSensor->getValue();
  // Nan check for when robot hasn't moved
  auto newLeftPos = isnan(currLeftPos) ? leftPos : currLeftPos - leftPos;
  auto newRightPos = isnan(currRightPos) ? rightPos : currRightPos - rightPos;
...
void Epuck::rotate90(char direction)
{
  ...
  do
  {
    ...
    leftEffective = _leftPosSensor->getValue();
    rightEffective = _rightPosSensor->getValue();
  } while (fabs(newLeftPos - leftEffective) > STEPDELTA || fabs(newRightPos - rightEffective) > STEPDELTA || fabs(yaw - effective) > 10*_imu->getNoise());
}
...
void Epuck::generatePaths()
{
  ...
  while (!queue.empty())
  {
    ...
    {
      if (cell == 189) {
        checkCell(cell, 172, queue, dist, parent);
      } else {
    ...
    {
      if (cell == 171) {
        checkCell(cell, 188, queue, dist, parent);
      } else {
        checkCell(cell, cell - 1, queue, dist, parent);
      }
    }

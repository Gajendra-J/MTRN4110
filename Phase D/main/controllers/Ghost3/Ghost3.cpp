    ...
    : _mapFileName(mapFileName), _wheelRadius(wheelRadius), _axleLength(axleLength)
{
  ...
  _startHeading = 'N';
  _translation = _robot->getFromDef("Ghost_3")->getField("translation");
...
void Epuck::getPosition() {
  ...
  const auto target = calculateCell(_target_translation->getSFVec3f());
  if (_adjacencyMatrix.at(target).at(0) == 1) {
    _end = target - _numCols;
  } else if (_adjacencyMatrix.at(target).at(1) == 1) {
    _end = target + 1;
  } else if (_adjacencyMatrix.at(target).at(2) == 1) {
    _end = target + _numCols;
  } else if (_adjacencyMatrix.at(target).at(3) == 1) {
    _end = target - 1;
  } else {
    _end = target;
  }
...
bool Epuck::checkCollision(double left, double right) {
  ...
  if (ps0 > 90 || ps1 > 90 || ps6 > 90 || ps7 > 90) {
    const auto values1 = _ghost1Translation->getSFVec3f();
    const auto values2 = _ghost2Translation->getSFVec3f();
    const auto curr = _translation->getSFVec3f();
    const auto distance1 = std::sqrt(std::pow(values1[0] - curr[0], 2) + std::pow(values1[2] - curr[2], 2));
    const auto distance2 = std::sqrt(std::pow(values2[0] - curr[0], 2) + std::pow(values2[2] - curr[2], 2));
